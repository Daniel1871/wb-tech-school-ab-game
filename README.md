...
- Я говорил ранее, что во время проведения эксперимента прибыль не меняется в зависимости от изменений, сделанных для тестовой группы ⇒ даже ужасное ухудшение конверсии в экспериментах мы можем не бояться, оно не говорит нам о том, что нужно резко все прерывать (ведь мы не несем убытков при проведении теста); длительные тесты не несут для нас финансовых потерь в этой игре (понятно, что если тест успешный, а мы долго держим, то можем не дозаработать). 
